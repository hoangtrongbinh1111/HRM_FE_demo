...
const SwitchBtn = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        const departmentId = props?.data?.departmentId === userData?.data?.department?.id ? props?.data?.departmentId : userData?.data?.department?.id;
        ...
        if (departmentId && fromPositionId && startPositionId) {
            getConfig({
                entity: props?.data?.approvalHistory?.[0]?.sign === 5 ||
                    props?.data?.approvalHistory?.[0]?.sign === 6 ?
                    props.entity + '-forward' : props.entity,
                        ...
                        <div className="mt-8 flex items-center justify-end ltr:text-right rtl:text-left">
                            {
                                !props.disable &&
                                <>
                                    {/* <RBACWrapper permissionKey={props.rbac} type={props.typeRbac}> */}
                            ...
                            {
                                (props.entity === "warehousingBillExport" || props.entity === "warehousingBillExportReturn" || props.entity === "warehousingBillImport") &&
                                <>
                                    {
                                        props.disable && props.data?.status === "IN_PROGRESS" && !props?.data?.exportStatus && !props?.data?.warehousingBill &&
                                        <RBACWrapper permissionKey={props.rbacFinish} type={props.typeRbacFinish}>
                                            <button data-testId="submit-btn" type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4 add-button" onClick={() => props.handleFinish()}>
                                                {t('export')}
                                            </button>
                                        </RBACWrapper>
                                    }
                                    {
                                        props.disable && props.data?.status === "PENDING" && props?.data?.warehousingBill?.exportStatus === 'RETURN' &&
                                        <RBACWrapper permissionKey={props.rbacFinish} type={props.typeRbacFinish}>
                                            <button data-testId="submit-btn" type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4 add-button" onClick={() => props.handleFinish()}>
                                                {t('import')}
                                            </button>
                                                    ...
                                                    signRole?.sign?.map((item: any) => {
                                                        return (
                                                            <>
                                                                {
                                                                    item.name === "REJECT" &&
                                                                    <button type="button" className="btn btn-outline-danger cancel-button w-28" onClick={() => props.handleReject()}>
                                                                        {t('reject')}
                                                                    </button>
                                                                }
                                                            </>
                            ...
                            }
                            {
                                signRole?.sign?.map((item: any) => {
                                    return (
                                        <>
                                            {
                                                item.name === 'SIGN' && btnStatus && (router.query.id === 'create' ? !props.disable : props.disable) &&
                                            ...
                                            {
                                                item.name === 'INITIAL' && btnStatus && props.disable &&
                                                <button data-testId="send-approval-btn" type="button" className="btn btn-primary add-button ltr:ml-4 rtl:mr-4" onClick={() => props.handleSubmitApproval(item.id)}>
                                                    {t('continue_initial')}
                                                </button>
                                            }
