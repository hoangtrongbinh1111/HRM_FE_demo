...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({ page: page, search: searchSuperior, departmentId: departmentId, documentType: 'repairRequest', positionId: profile?.data.position.id });
    ...
    const handleRepair = (param: any) => {
        ...
        if (props.data?.approvalHistory?.length > 0) {
            RepairForward({ id: router.query.id, ...query }).then(() => {
            ...
            RepairSubmit({ id: router.query.id !== 'create' ? router.query.id : props.id, ...query }).then(() => {
                showMessage(`${t('forward_success')}`, 'success');
