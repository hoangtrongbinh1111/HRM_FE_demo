...
import { OrderForward, OrderSubmit } from '@/services/apis/order.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleRepair = (param: any) => {
        ...
        if (props.data?.approvalHistory?.length > 0) {
            OrderForward({ id: router.query.id, ...query }).then(() => {
            ...
            OrderSubmit({ id: router.query.id !== 'create' ? router.query.id : props.id, ...query }).then(() => {
                showMessage(`${t('forward_success')}`, 'success');
