...
import { WarehousingBillForwardReturn, WarehousingBillSubmitReturn } from '@/services/apis/warehousing-bill-return.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        documentType: 'warehousingBillExportReturn',
        page: page,
    ...
    const handleRepair = (param: any) => {
        ...
        if (props?.data?.approvalHistory?.length > 0 && router.query.id !== "create") {
            WarehousingBillForwardReturn({ id: props.id, ...query }).then(() => {
            ...
            WarehousingBillSubmitReturn({ id: props.id, ...query }).then(() => {
                showMessage(`${t('forward_success')}`, 'success');
