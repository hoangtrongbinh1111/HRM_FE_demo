...
import { WarehousingBillApprove, WarehousingBillReject } from '@/services/apis/warehousing-bill.api';
...
import { WarehousingBillReturn } from '@/services/swr/warehousing-bill-return.swr';
import { DeleteWarehousingBillReturn } from '@/services/apis/warehousing-bill-return.api';
...
const WarehousingPage = ({ ...props }: Props) => {
    ...
    // get data
    const { data: warehousingReturn, pagination, mutate, isLoading } = WarehousingBillReturn({
        ...router.query, warehouseId: active.includes(0) ? "" : active, sortBy: "id.DESC",
    });
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            ...
            .then((result) => {
                if (result.value) {
                    DeleteWarehousingBillReturn({ id }).then(() => {
                        mutate();
    ...
    const handleDetail = (value: any) => {
        router.push(`/warehouse-process/warehousing-bill/return/${value.id}?status=${value.status}`)
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: ({ warehousingBill: { proposal, order, repairRequest } }: any) => <span> {proposal?.name || order?.name || repairRequest?.name} </span>,
        },
        {
            accessor: 'code',
            title: `${t('code')}`,
            render: ({ warehousingBill: { code } }: any) => <span> {code}</span>,
        },
        {
            accessor: 'warehouse',
            title: `${t('warehouse_name')}`,
            render: ({ warehousingBill }: any) => {
                return <span>{warehousingBill?.warehouse?.name}</span>
            },
        },
        {
            accessor: 'status',
                    ...
                    <div className="flex justify-center gap-2 mr-2">
                        <RBACWrapper
                            ...
                            <div className="w-[auto]">
                                <Link href={`/warehouse-process/warehousing-bill/return/${records.id}?status=${true}`}>
                        ...
                        {
                            records.status === "DRAFT" && records?.createdById === userData?.data?.id &&
                            <div className="w-[auto]">
            ...
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                        ...
                        <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                            <span>{t('warehousing_bill_return')}</span>
                ...
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        <button data-testId="add-export" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/warehouse-process/warehousing-bill/return/create`)}>
                            <IconNewPlus />
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        records={warehousingReturn?.data}
