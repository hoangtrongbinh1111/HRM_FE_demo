...
import { RepairForward, RepairSubmit } from '@/services/apis/repair.api';
import { useProfile } from '@/services/swr/profile.swr';
import { WarehousingBillForward, WarehousingBillSubmit } from '@/services/apis/warehousing-bill.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        documentType: props?.data?.status === 'RETURN' || props?.data?.status === 'IN_PROGRESS_RETURN' ? 'warehousingBillExportReturn' : 'warehousingBillExport',
        page: page,
    ...
    const handleRepair = (param: any) => {
        ...
        if (props.data?.approvalHistory?.length > 0 && props?.data.status !== 'RETURN') {
            WarehousingBillForward({ id: router.query.id, ...query }).then(() => {
            ...
            WarehousingBillSubmit({ id: router.query.id !== 'create' ? router.query.id : props.id, ...query }).then(() => {
                showMessage(`${t('forward_success')}`, 'success');
