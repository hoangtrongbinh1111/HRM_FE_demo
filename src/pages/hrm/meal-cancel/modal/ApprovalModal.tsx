...
import { MealCancelForward } from '@/services/apis/meal-cancel.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        MealCancelForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/meal-cancel/${props.createdId}`)
        }
    };
