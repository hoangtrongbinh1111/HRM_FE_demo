...
// API
import { RequestOvertimes } from '@/services/swr/request-overtime.swr';
import { DeleteRequestOvertime, RequestOvertimeApprove, RequestOvertimeReject } from '@/services/apis/request-overtime.api';
...
const RequestOvertimePage = ({ ...props }: Props) => {
    ...
    const { data: requestOvertime, pagination, mutate, isLoading
    } = RequestOvertimes({ sortBy: "id.DESC", ...router.query });
    ...
    useEffect(() => {
        dispatch(setPageTitle(`${t('request_overtime')}`));
    });

    useEffect(() => {
        if (typeof window !== 'undefined') {
            setProfile(localStorage.getItem('profile') && JSON.parse(localStorage.getItem('profile') || ''))
    ...
    useEffect(() => {
        if (requestOvertime?.data.length <= 0 && pagination.page > 1) {
            router.push({
                query: {
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_request_overtime')}`,
                text: `${t('delete')} ${t('request_overtime')}`,
                padding: '2em',
    ...
    const handleDetail = (value: any) => {
        router.push(`/hrm/request-overtime/${value.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`)
    };
    ...
    const columns = [
        ...
        {
            accessor: 'registerday', title: `${t('registerday')}`, sortable: false, render: (records: any, index: any) => <span onClick={(records) => handleDetail(records)}>{`${dayjs(records?.registrationDay).format("DD/MM/YYYY")}`}</span>
        },
        ...
        },
        {
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    {/* <RBACWrapper
                        permissionKey={[
                            'requestOvertime:findOne',
                            'requestOvertime:headApprove',
                            'requestOvertime:headReject',
                        ]}
                        type={'OR'}
                    > */}
                    <div className="w-[auto]">
                        <Link href={`/hrm/request-overtime/${records.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`}>
                            <button data-testId="detail-request-overtime-btn" type='button' className='button-detail'>
                    ...
                    <RBACWrapper
                        permissionKey={[
                            'requestOvertime:exportTextDraft',
                        ]}
                        type={'OR'}
                    >
                        {
                    ...
                    {/* </RBACWrapper> */}
                    {
                        ...
                        <div className="w-[auto]">
                            <button data-testId="edit-request-overtime-btn" type="button" className='button-edit' onClick={() => handleDetail(records)}>
                                <IconNewEdit /><span>
                                    {t('edit')}
                                </span>
                            </button>
                    ...
                    }
                    {
                        ...
                        // <RBACWrapper permissionKey={['requestOvertime:remove']} type={'AND'}>
                        <div className="w-[auto]">
            ...
            <div className="panel mt-6">
                ...
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper permissionKey={['requestOvertime:create']} type={'AND'}> */}
                        <button data-testId="add-request-overtime" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/hrm/request-overtime/create?page=${pagination?.page}&perPage=${pagination?.perPage}`)}>
                            <IconNewPlus />
...
export default RequestOvertimePage;
