...
import { PaymentOrders } from '@/services/swr/payment-order.swr';
...
import { DeletePaymentOrder } from '@/services/apis/payment-order.api';
...
const LateEarlyForm = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(`${t('payment_order')}`));
    });
    ...
    const { data: paymentOrder, pagination, mutate } = PaymentOrders({ sortBy: 'id.DESC', ...router.query });
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/payment-order/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/payment-order/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const handleDelete = (data: any) => {
        ...
        swalDeletes
            ...
            .then((result) => {
                if (result.value) {
                    DeletePaymentOrder({ id: data?.id }).then(res => {
                        mutate();
                ...
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        <RBACWrapper
                            ...
                            type={'AND'}
                        >
                            <Link href={`/hrm/payment-order/create?page=${pagination?.page}&perPage=${pagination?.perPage}`}>
