...
import { PaymentOrderForward, PaymentOrderPending } from '@/services/apis/payment-order.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        PaymentOrderForward({ id: id_, ...dataSubmit }).then(() => {
            showMessage(`${t('forward_success')}`, 'success');
            // props.handleData();
            props.handleCancel();
            // handleCancel();
            router.push(`/hrm/payment-order`)
        }).catch((err) => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/payment-order/${props.createdId}`)
        }
    };
