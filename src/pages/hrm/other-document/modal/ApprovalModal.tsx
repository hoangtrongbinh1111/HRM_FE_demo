...
import { OtherDocumentForward, OtherDocumentPending } from '@/services/apis/other-document.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        OtherDocumentForward({ id: id_, ...dataSubmit }).then(() => {
            showMessage(`${t('forward_success')}`, 'success');
            // props.handleData();
            props.handleCancel();
            // handleCancel();
            router.push(`/hrm/other-document`)
        }).catch((err) => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/other-document/${props.createdId}`)
        }
    };
