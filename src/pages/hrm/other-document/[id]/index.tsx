...
import { OtherDocumentApprove, CreateOtherDocument, GetOtherDocument, EditOtherDocument } from '@/services/apis/other-document.api';
...
const DetailPage = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(disable ? `${t('Detail other form')}` : (data ? t('Update other form') : t('Add other form'))));
    });
    ...
    const handleData = () => {
        if (Number(router.query.id)) {
            GetOtherDocument({ id: router.query.id })
                .then((res) => {
                    setData(res.data);
                    setIsLoading(false);
                })
    ...
    const handleCancel = () => {
        router.push(`/hrm/other-document?page=${router?.query?.page}&perPage=${router?.query?.perPage}`);
    };
    ...
    const handleOtherDocument = (param: any) => {
        ...
        if (path) {
            ...
            showMessage(`${t('please_choose_file')}`, 'error');
            return
        }
        if (data) {
            EditOtherDocument({ id: data?.id, ...dataSubmit }).then(res => {
            ...
            CreateOtherDocument(dataSubmit).then(res => {
    ...
    const Approve = () => {
        OtherDocumentApprove({ id: router.query.id })
            .then(() => {
                handleCancel();
    ...
    const handleApprove = () => {
        ...
        swalDeletes
            .fire({
                // icon: 'question',
                title: `${t('approve')}`,
                text: `${t('approve')} ${t('otherDocument')}`,
                padding: '2em',
    ...
    //     setLoading(true)
    //     downloadFile("payment_order.pdf", `/other-document/${id}/export-text-draft`).finally(() => {
    //         setLoading(false)
    //     })
    // }
    return (
        <div>
            ...
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <Link href="/hrm/other-form" className="text-primary hover:underline">
                        <span>{t('Other form')}</span>
                    </Link>

                </li>
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <span>
                        {!disable && (data ? t('Update other form') : t('Add other form'))}
                        {
                            disable && t('Detail other form')
                        }
                    </span>
                </li>
            </ul>
                ...
                <div className="flex" style={{ alignItems: 'center' }}>
                    {(data?.status !== "DRAFT" && data?.status !== "REJECTED" && data?.status !== "APPROVED") && ((data?.status === "PENDING" || data?.status === "IN_PROGRESS") && userData?.data?.id === data?.currentApprover?.id) &&
                        disable && Number(data?.createdById) !== userData?.data?.id && (
                            // <RBACWrapper permissionKey={['orderForm:update']} type={'AND'}>
                                ...
                                >
                                    {({ errors, values, submitCount, setFieldValue }) => (
                                        <Form className="space-y-5 p-4">
                                            ...
                                            <div className='mt-5'>
                                                <label htmlFor="attachmentIds" className='label'> {t('attached_file')} <span style={{ color: 'red' }}>* </span></label >
                                                <Field
                                                    innerRef={fileRef}
                                                    autoComplete="off"
                                                    name="attachmentIds"
