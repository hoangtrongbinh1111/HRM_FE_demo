...
// API
// constants
import { PAGE_SIZES, PAGE_SIZES_DEFAULT, PAGE_NUMBER_DEFAULT, LIST_STATUS_COLOR, LIST_STATUS, LIST_STATUS_MEAN } from '@/utils/constants';
// helper
...
import { OtherDocuments } from '@/services/swr/other-document.swr';
...
import { DeleteOtherDocument } from '@/services/apis/other-document.api';
...
const LateEarlyForm = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(`${t('Other form')}`));
    });
    ...
    const { data: otherDocument, pagination, mutate } = OtherDocuments({ sortBy: 'id.DESC', ...router.query });
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/other-document/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/other-document/${data.id}?status=${true}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const handleDelete = (data: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_form')}`,
                html: `<span class='confirm-span'>${t('delete_payment_order')}</span>?`,
                padding: '2em',
            ...
            .then((result) => {
                if (result.value) {
                    DeleteOtherDocument({ id: data?.id }).then(res => {
                        mutate();
        ...
        },
        {
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    ...
                    {
                        (records.status === LIST_STATUS.DRAFT || records?.approvalHistory?.length <= 1) &&
                        <div className="w-[auto]">
                ...
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper
                            ...
                            type={'AND'}
                        > */}
                            <Link href="/hrm/other-document/create">
