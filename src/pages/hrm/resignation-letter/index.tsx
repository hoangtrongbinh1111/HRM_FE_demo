...
import { ResignationLetters } from '@/services/swr/resignation-letter.swr';
import { DeleteResignationLetter } from '@/services/apis/resignation-letter.api';
...
const ResignationLetterPage = ({ ...props }: Props) => {
    ...
    const { data: resignationLetter, pagination, mutate, isLoading } = ResignationLetters({ sortBy: "id.DESC", ...router.query });

    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        dispatch(setPageTitle(`${t('resignation_letter')}`));
    });

    useEffect(() => {
        if (resignationLetter?.data.length <= 0 && pagination.page > 1) {
            router.push({
                query: {
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_resignation_letter')}`,
                text: `${t('delete')} ${name}`,
                padding: '2em',
            ...
            .then((result) => {
                if (result.value) {
                    DeleteResignationLetter({ id }).then(() => {
                        mutate();
    ...
    const handleDetail = (value: any) => {
        router.push(`/hrm/resignation-letter/${value.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleEdit = (value: any) => {
        router.push(`/hrm/resignation-letter/${value.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const columns = [
        ...
        { accessor: 'name', title: `${t('personel_name')}`, render: ({ createdBy }: any, index: any) => <span>{createdBy?.fullName}</span>, sortable: false },
        {
            accessor: 'deparment',
            title: `${t('department')}`,
            render: ({ createdBy }: any, index: any) => <span>{createdBy?.department?.name}</span>, sortable: false
        },
        {
            accessor: 'resignationDay',
            title: `${t('resignationDay')}`,
            render: (records: any, index: any) => <span>{moment(records.resignationDay).format('DD-MM-YYYY')}</span>, sortable: false
        },
        {
            accessor: 'createdAt',
            title: `${t('Requested Date')}`,
            render: (records: any, index: any) => <span>{moment(records.createdAt).format('DD-MM-YYYY')}</span>, sortable: false
        },
        ...
        },
        {
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    {/* <RBACWrapper
                        permissionKey={[
                            'resignationLetter:findOne',
                            'resignationLetter:headApprove',
                            'resignationLetter:headReject',
                        ]}
                        type={'OR'}
                    > */}
                    <div className="w-[auto]">
                        <Link href={`/hrm/resignation-letter/${records.id}?status=${true}`}>
                    ...
                    <RBACWrapper
                        permissionKey={[
                            'resignationLetter:exportTextDraft',
                        ]}
                        type={'OR'}
                    >
                        {
                    ...
                    </RBACWrapper>
                    {
                        ...
                        // <RBACWrapper permissionKey={['resignationLetter:update']} type={'AND'}>
                        <div className="w-[auto]">
                    ...
                    }
                    {
                        ...
                        // <RBACWrapper permissionKey={['resignationLetter:remove']} type={'AND'}>
                        <div className="w-[auto]">
            ...
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper permissionKey={['resignationLetter:create']} type={'AND'}> */}
                        <button data-testId="add-proposal" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/hrm/resignation-letter/create?page=${pagination?.page}&perPage=${pagination?.perPage}`)}>
                            <IconNewPlus />
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        records={resignationLetter?.data}
                        columns={columns}
