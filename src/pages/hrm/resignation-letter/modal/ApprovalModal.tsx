...
import { ResignationLetterForward, ResignationLetterInitial, ResignationLetterPending } from '@/services/apis/resignation-letter.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        ResignationLetterForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/resignation-letter/${props.createdId}`)
        }
    };
