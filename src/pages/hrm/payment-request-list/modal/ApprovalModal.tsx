...
import { PaymentRequestListForward, PaymentRequestListPending } from '@/services/apis/payment-request-list.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        documentType: props.forward ? 'payment-request-list-forward' : 'payment-request-list',
        page: page,
    ...
    const handleProposal = (param: any) => {
        ...
        PaymentRequestListForward({ id: id_, ...dataSubmit }).then(() => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/payment-request-list/${props.createdId}`)
        }
    };
