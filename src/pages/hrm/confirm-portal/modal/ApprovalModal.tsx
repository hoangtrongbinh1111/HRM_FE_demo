...
import { ConfirmPortalForward, ConfirmPortalPending } from '@/services/apis/confirm-portal.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        ...
        documentType: props.have ? 'confirm-portal-have' : 'confirm-portal'
    });
    ...
    const handleProposal = (param: any) => {
        ...
        ConfirmPortalForward({ id: id_, ...dataSubmit }).then(() => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/confirm-portal/${props.createdId}`)
        }
    };
