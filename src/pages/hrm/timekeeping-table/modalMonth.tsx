...
const ModalMonth = ({ ...props }: Props) => {
    ...
    const handleExportFile = () => {
        if (!selectedMonth) {
            ...
            setLoading(true);
            const [month_, year_] = selectedMonth?.split('/');
            ...
            if (props?.keyDownload === 'timekeeping-staff') {
                const filename = `${t(`file_${props?.keyDownload}`)}_${currentDate}`
            ...
            } else if (props?.keyDownload === 'time-keeping') {
                const filename = `${t(`file_time_keeping_report`)}_${currentDate}`
                downloadFile(`${filename}.xlsx`, `/time-keeping/export?${stringQuery}`).finally(() => {
                    setDepartmentId([])
                    props?.setOpenModal(false)
                    setLoading(false);
                })
            } else {
                const filename = `${t(`file_timekeeping_staff_v2`)}_${currentDate}`
