...
import { ForgotCheckinoutForward, ForgotCheckinoutPending } from '@/services/apis/forgot-checkin-out.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        ...
        documentType: 'forgot-checkin-out'
    });
    ...
    const handleProposal = (param: any) => {
        ...
        ForgotCheckinoutForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/forgot-checkin-out/${props.createdId}`)
        }
    };
