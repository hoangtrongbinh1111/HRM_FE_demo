...
import { LeaveApplications } from '@/services/swr/leave-application.swr';
import { listAllDepartmentTree } from '@/services/apis/department.api';
import { flattenDepartments } from '@/utils/commons';
import { DeleteLeaveApplication } from '@/services/apis/leave-application.api';
...
const LateEarlyForm = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(`${t('leave_application')}`));
    });
    ...
    const { data: leaveApplication, pagination, mutate } = LeaveApplications({ sortBy: 'id.DESC', ...router.query });
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/leave-application/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/leave-application/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const handleDelete = (data: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_form')}`,
                html: `<span class='confirm-span'>${t('delete_leave_application')}</span>`,
                padding: '2em',
            ...
            .then((result) => {
                if (result.value) {
                    DeleteLeaveApplication({ id: data?.id }).then(res => {
                        mutate();
        ...
        },
        {
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    ...
                    <RBACWrapper
                        permissionKey={[
                            'leaveApplication:exportTextDraft',
                        ]}
                        type={'OR'}
                    >
                        {
                ...
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper
                            ...
                            type={'AND'}
                        > */}
                        <Link href={`/hrm/leave-application/create?page=${pagination?.page}&perPage=${pagination?.perPage}`}>
