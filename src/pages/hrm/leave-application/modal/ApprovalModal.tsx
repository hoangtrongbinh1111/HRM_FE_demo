...
import { LeaveApplicationForward, LeaveApplicationInitial, LeaveApplicationPending } from '@/services/apis/leave-application.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        LeaveApplicationForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/leave-application/${props.createdId}`)
        }
    };
