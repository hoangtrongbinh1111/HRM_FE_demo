...
import { RiceCouponDetails } from '@/services/swr/rice-coupon.swr';
import { AddRiceCouponDetails, CreateRiceCoupon, DeleteRiceCouponDetail, EditRiceCoupon, GetRiceCoupon, RiceCouponApprove } from '@/services/apis/rice-coupon.api';
...
const DetailPage = ({ ...props }: Props) => {
    ...
    const handleExportFile = (id: any) => {
        setLoading(true)
        downloadFile("rice_coupon.pdf", `/rice-coupon/${id}/export-text-draft`).finally(() => {
            setLoading(false)
        })
    }
    useEffect(() => {
        dispatch(setPageTitle(disable ? `${t('detail_rice_coupon')}` : (data ? t('update_rice_coupon') : t('add_rice_coupon'))));
    });
    ...
    const handleData = () => {
        GetRiceCoupon({ id: router.query.id })
            .then((res) => {
                setData(res.data);
            })
            .catch((err) => {
    ...
    const handleRiceCoupon = (param: any) => {
        ...
        if (data) {
            EditRiceCoupon({ id: data?.id, ...query })
                .then((res) => {
    ...
    const Approve = () => {
        ...
        RiceCouponApprove({ id: router.query.id, sign: 3 }).then(() => {
            handleCancel();
    ...
    const handleDownloadTemplate = () => {
        ...
        switch (lang) {
            case "vi":
                urlString = `${process.env.NEXT_PUBLIC_APP_URL}/templates/rice_coupon/rice_coupon_vi.xlsx`;
                break;
            case "la":
                urlString = `${process.env.NEXT_PUBLIC_APP_URL}/templates/rice_coupon/rice_coupon_la.xlsx`;
                break;
            case "en":
                urlString = `${process.env.NEXT_PUBLIC_APP_URL}/templates/rice_coupon/rice_coupon_en.xlsx`;
                break;
            default:
                urlString = `${process.env.NEXT_PUBLIC_APP_URL}/templates/rice_coupon/rice_coupon_vi.xlsx`;
                break;
        ...
        link.href = urlString;
        link.setAttribute('download', 'rice_coupon.xlsx');
        ...
        <div>
            ...
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <Link href="/hrm/rice-coupon" className="text-primary hover:underline">
                        <span>{t('rice_coupon')}</span>
                    </Link>

                </li>
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <span>
                        {!disable && (data ? t('update_rice_coupon') : t('add_rice_coupon'))}
                        {
                            disable && t('detail_rice_coupon')
                        }
                    </span>
                </li>
            </ul>
                ...
                <div className="flex" style={{ alignItems: 'center' }}>
                    ...
                    {/* {
                        disable && (data?.status === LIST_STATUS.DRAFT || (userData?.data?.id === data?.currentApproverId && data?.status !== LIST_STATUS.APPROVED && data?.status !== LIST_STATUS.REJECTED)) && (
                            <Link href={`/hrm/rice-coupon/${router?.query.id}?page=${router?.query?.page}&perPage=${router?.query?.perPage}`}>
                                <button className="edit-page-btn btn btn-primary h-9 ltr:ml-4 rtl:mr-4">{t('edit')}</button>
                            </Link>
                        )} */}
                    <Link href={`/hrm/rice-coupon`}>
