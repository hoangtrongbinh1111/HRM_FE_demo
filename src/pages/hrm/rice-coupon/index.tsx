...
import { RiceCoupons } from '@/services/swr/rice-coupon.swr';
import { DeleteRiceCoupon } from '@/services/apis/rice-coupon.api';
...
const RiceCouponPage = ({ ...props }: Props) => {
    ...
    const { data: riceCoupon, pagination, mutate, isLoading } = RiceCoupons({ sortBy: "id.DESC", ...router.query });

    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        dispatch(setPageTitle(`${t('rice_coupon')}`));
    });
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_rice_coupon')}`,
                text: `${t('delete')} ${t('rice_coupon')}`,
                padding: '2em',
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/rice-coupon/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/rice-coupon/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    ...
                    <div className="w-[auto]">
                        <Link href={`/hrm/rice-coupon/${records.id}?status=${true}`}>
            ...
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        ...
                        <button data-testId="add-proposal" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/hrm/rice-coupon/create?page=${pagination?.page}&perPage=${pagination?.perPage}`)}>
                            <IconNewPlus />
