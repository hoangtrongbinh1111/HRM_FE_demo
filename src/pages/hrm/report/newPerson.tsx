...
import { NewPerson } from '@/services/swr/report.swr';
...
const NewPersonPage = ({ ...props }: Props) => {
    ...
    const { data: newPerson, pagination, mutate, isLoading } = NewPerson({ ...router.query, startDay: dayCurrent, endDay: dayBefor });
    ...
    const columns = [
        ...
        },
        {
            accessor: 'fullName',
            title: `${t('name_staff')}`,
            render: ({ fullName }: any) => <span>{fullName}</span>,
        },
        {
            accessor: 'department',
            title: `${t('department')}`,
        ...
        },
        {
            accessor: 'createdAt',
            title: `${t('startDate')}`,
            render: ({ createdAt }: any) => <span>{moment(createdAt).format("DD/MM/YYYY")}</span>,
        },
    ]
    ...
    const handleExportFile = () => {
        ...
        setIsDisabled(true)
        downloadFile("new-staff.xlsx", `/report/new-person/generate-xlsx?startDay=${dayCurrent}&endDay=${dayBefor}`).finally(() => {
