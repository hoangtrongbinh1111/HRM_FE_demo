...
import { NewPerson, OnLeave } from '@/services/swr/report.swr';
...
const OnLeavePage = ({ ...props }: Props) => {
    ...
    const { data: onLeave, pagination, mutate, isLoading } = OnLeave({ ...router.query, startDay: dayCurrent, endDay: dayBefor });
    ...
    const columns = [
        ...
        },
        {
            accessor: 'createdAt',
            title: `${t('startDay')}`,
            render: ({ startDay }: any) => <span>{moment(startDay).format("DD/MM/YYYY hh:mm")}</span>,
        },
        {
            accessor: 'createdAt',
            title: `${t('endDay')}`,
            render: ({ endDay }: any) => <span>{moment(endDay).format("DD/MM/YYYY hh:mm")}</span>,
        },
    ]
    ...
    const handleExportFile = () => {
        ...
        setIsDisabled(true)
        downloadFile("on-leave.xlsx", `/report/on-leave/generate-xlsx?startDay=${dayCurrent}&endDay=${dayBefor}`).finally(() => {
