...
import { RequestAdditionalPersonnels } from '@/services/swr/request-additional-personnel.swr';
import { DeleteRequestAdditionalPersonnel } from '@/services/apis/request-additional-personnel.api';
...
const RequestAdditionalPersonnelPage = ({ ...props }: Props) => {
    ...
    const { data: requestAdditionalPersonnel, pagination, mutate, isLoading } = RequestAdditionalPersonnels({ sortBy: "id.DESC", ...router.query });

    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        dispatch(setPageTitle(`${t('requestAdditionalPersonnel')}`));
    });

    useEffect(() => {
        if (requestAdditionalPersonnel?.data.length <= 0 && pagination.page > 1) {
            router.push({
                query: {
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            .fire({
                ...
                text: `${t('delete')} ${t('requestAdditionalPersonnel')}`,
                padding: '2em',
            ...
            .then((result) => {
                if (result.value) {
                    DeleteRequestAdditionalPersonnel({ id }).then(() => {
                        mutate();
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/request-additional-personnel/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/request-additional-personnel/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const columns = [
        ...
        {
            accessor: 'createdAt', title: `${t('time')}`, render: (records: any, index: any) => <span>{moment(records.createdAt).format('DD-MM-YYYY')}</span>, sortable: false
        },
        {
        ...
        {
            accessor: 'deparment', title: `${t('Request Department')}`, render: ({ department }: any, index: any) => <span>{department?.name}</span>, sortable: false
        },
        ...
        },
        {
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    {/* <RBACWrapper
                        permissionKey={[
                            'requestAdditionalPersonnel:findOne',
                            'requestAdditionalPersonnel:headApprove',
                            'requestAdditionalPersonnel:headReject',
                        ]}
                        type={'OR'}
                    > */}
                    <div className="w-[auto]">
                        <Link href={`/hrm/request-additional-personnel/${records.id}?status=${true}`}>
                    ...
                    {/* <RBACWrapper
                        permissionKey={[
                            'requestAdditionalPersonnel:exportTextDraft',
                        ]}
                        type={'OR'}
                    >
                        {
                    ...
                    }
                    {
                        ...
                        <RBACWrapper permissionKey={['requestAdditionalPersonnel:remove']} type={'AND'}>
                            <div className="w-[auto]">
            ...
            <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                <ul className="flex space-x-2 rtl:space-x-reverse mb-1">
                    ...
                    <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                        <span>{t('requestAdditionalPersonnel')}</span>
                    </li>
                </ul>
            </div>
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper permissionKey={['requestAdditionalPersonnel:create']} type={'AND'}> */}
                            <button data-testId="add-proposal" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/hrm/request-additional-personnel/create?page=${pagination?.page}&perPage=${pagination?.perPage}`)}>
                                <IconNewPlus />
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        records={requestAdditionalPersonnel?.data}
                        columns={columns}
...
export default RequestAdditionalPersonnelPage;
