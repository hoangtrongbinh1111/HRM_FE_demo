...
import { RequestAdditionalPersonnelForward, RequestAdditionalPersonnelPending } from '@/services/apis/request-additional-personnel.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        RequestAdditionalPersonnelForward({ id: id_, ...dataSubmit }).then(() => {
            showMessage(`${t('forward_success')}`, 'success');
            // props.handleData();
            props.handleCancel();
            // handleCancel();
            router.push(`/hrm/request-additional-personnel`)
        }).catch((err) => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/request-additional-personnel/${props.createdId}`)
        }
    };
                                                ...
                                                <Form className="space-y-5" >
                                                    ...
                                                    <div className="mt-8 flex items-center justify-end ltr:text-right rtl:text-left">
                                                        ...
                                                        <button data-testId="submit-modal-btn" type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4 add-button" onClick={() => handleSubmit()}>
                                                            {isSubmit ? <Loader size="sm" /> : t('continue_approval')}
                                                        </button>
                                                    </div>
                                                </Form>
                                            )}
