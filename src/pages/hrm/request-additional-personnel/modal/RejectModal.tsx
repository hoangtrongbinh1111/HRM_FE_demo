...
import { ProposalReject } from '@/services/apis/proposal.api';
import { RequestAdditionalPersonnelReject } from '@/services/apis/request-additional-personnel.api';
...
const RejectModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        setIsReject(true);
        RequestAdditionalPersonnelReject({ id: router.query.id, comment: param.comment }).then(() => {
            handleCancel();
                                        ...
                                        >
                                            {({ errors, values, setFieldValue, submitCount }) => (
                                                <Form className="space-y-5" >
                                                    <div className="mb-5">
                                                        <label htmlFor="comment" className="label" > {t('description')} <span style={{ color: 'red' }}>* </span></label >
                                                    ...
                                                    <div className="mt-8 flex items-center justify-end ltr:text-right rtl:text-left">
                                                        ...
                                                        <button data-testId="submit-modal-btn" type="submit" className="btn btn-primary ltr:ml-4 rtl:mr-4 add-button">
                                                            {isReject ? <Loader size="sm" /> : `${props.data !== undefined ? t('update') : t('reject')}`}
                                                        </button>
                                                    </div>
                                                </Form>
                                            )}
