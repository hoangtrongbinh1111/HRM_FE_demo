...
import { RequestShiftChangeForward } from '@/services/apis/request-shift-change.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        ...
        documentType: 'request-shift-change'
    });
    ...
    const handleProposal = (param: any) => {
        ...
        RequestShiftChangeForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
            ...
            props.handleCancel();
            router.push(`/hrm/request-shift-change?page=${router?.query?.page ?? 1}&perPage=${router?.query?.perPage ?? 10}`)
        }).catch((err) => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/request-shift-change/${props.createdId}`)
        }
    };
