...
import { GuestNoticeForward } from '@/services/apis/guest-notice.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const handleProposal = (param: any) => {
        ...
        GuestNoticeForward({ id: id_, ...dataSubmit }).then(() => {
            if (props.sign === 2) {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/guest-notice/${props.createdId}`)
        }
    };
