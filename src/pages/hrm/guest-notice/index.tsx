...
import { GuestNotices } from '@/services/swr/guest-notice.swr';
import { DeleteGuestNotice } from '@/services/apis/guest-notice.api';
...
const GuestNoticePage = ({ ...props }: Props) => {
    ...
    const { data: riceCoupon, pagination, mutate, isLoading } = GuestNotices({ sortBy: "id.DESC", ...router.query });

    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        dispatch(setPageTitle(`${t('guest_notice')}`));
    });
    ...
    const handleDelete = ({ id, name }: any) => {
        ...
        swalDeletes
            .fire({
                title: `${t('delete_guest_notice')}`,
                text: `${t('delete')} ${t('guest_notice')}`,
                padding: '2em',
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/guest-notice/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/guest-notice/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
    ...
    const columns = [
        ...
        {
            accessor: 'checker', title: `${t('checker')}`, sortable: false, render: (records: any, index: any) => {
                return <span>{records?.currentApprover?.fullName || records?.approvalHistory?.[0]?.approver?.fullName}</span>
            }
        },
            ...
            render: (records: any) => (
                <div className="flex justify-center gap-2">
                    ...
                    <div className="w-[auto]">
                        <Link href={`/hrm/guest-notice/${records.id}?status=${true}`}>
            ...
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        ...
                        <button data-testId="add-proposal" type="button" className="m-1 button-table button-create" onClick={(e) => router.push(`/hrm/guest-notice/create?page=${pagination?.page}&perPage=${pagination?.perPage}`)}>
                            <IconNewPlus />
...
export default GuestNoticePage;
