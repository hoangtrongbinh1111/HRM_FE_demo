...
import { convertDateFormat2, formatStartDate, formatEndDate, formatDate2, flattenDepartments } from '@/utils/commons';
// API
...
import { CalendarDepartments } from '@/services/swr/calendarDepartment.swr';
...
const Calendar = ({ ...props }: Props) => {
	...
	const { data: announcements, pagination, mutate } = CalendarDepartments({ sortBy: 'id.DESC', ...router.query });
	let user: any;
	...
	const columns = [
		...
		},
		{
			...
			title: `${t('position')}`,
			sortable: false,
			render: (record: any) => <span>{record?.position?.name} </span>,
		},
		{ accessor: 'status', title: `${t('task')}`, sortable: false, render: (record: any) => <span>{record?.assignedTasks[0]?.name}</span> },
		
		{
			accessor: 'notes',
			title: `${t('work_schedule')}`,
			sortable: false,
			render: (record: any) => <span></span>,
		},
		// {
		// 	accessor: 'action',
		// 	title: `${t('action')}`,
