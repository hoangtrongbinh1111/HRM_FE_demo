...
import { RequestAdvancePaymentForward, RequestAdvancePaymentPending } from '@/services/apis/request-advance-payment.api';
...
const ApprovalModal = ({ ...props }: Props) => {
    ...
    const { data: superiorDropdown, pagination: superiorPagination, isLoading: superiorLoading } = DropdownSuperior({
        documentType: props.forward ? 'request-advance-payment-forward' : 'request-advance-payment',
        page: page,
    ...
    const handleProposal = (param: any) => {
        ...
        RequestAdvancePaymentForward({ id: id_, ...dataSubmit }).then(() => {
    ...
    const handleCancel = () => {
        ...
        if (props.createdId) {
            router.push(`/hrm/request-advance-payment/${props.createdId}`)
        }
    };
                                                        ...
                                                        <button data-testId="submit-modal-btn" type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4 add-button" onClick={() => handleSubmit()}>
                                                            {
                                                                isSubmit ? <Loader size="sm" /> :
                                                                    props.sign === 2 ? t('continue_approval') :
                                                                        props?.data?.approvalHistory?.[0]?.sign === 5 ? t('continue') :
                                                                            props.forward ? t('forward') :
                                                                                t('continue_initial')
                                                            }
                                                        </button>
                                                    </div>
