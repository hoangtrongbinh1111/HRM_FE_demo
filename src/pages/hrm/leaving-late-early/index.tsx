...
const LateEarlyForm = ({ ...props }: Props) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle(`${t('leaving_late_early')}`));
    });
    ...
    const { data: leavingLateEarly, pagination, mutate } = LeavingLateEarlys({ sortBy: 'id.DESC', ...router.query });
    ...
    const handleEdit = (data: any) => {
        router.push(`/hrm/leaving-late-early/${data.id}?page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };

    const handleDetail = (data: any) => {
        router.push(`/hrm/leaving-late-early/${data.id}?status=${true}&page=${pagination?.page}&perPage=${pagination?.perPage}`);
    };
        ...
        swalDeletes
            ...
            .then((result) => {
                if (result.value) {
                    ...
                    DeleteLeavingLateEarly({ id: data?.id }).then(res => {
                        mutate();
    ...
    const columns = [
        ...
        {
            accessor: 'registerday', title: `${t('registerday')}`, sortable: false, render: (records: any, index: any) => <span>{records?.registrationDay ? toDateString(records?.registrationDay) : ''}</span>
        },
        {
            accessor: 'checker',
            title: `${t('checker')}`,
            sortable: false,
            render: (records: any, index: any) => <span>{records?.currentApprover?.fullName}</span>
        },
        {
            accessor: 'isCheck',
            title: `${t('status')}`,
                        ...
                        (records.status === LIST_STATUS.DRAFT || records.status === LIST_STATUS.IN_PROGRESS) && userData?.data?.id === records?.createdById &&
                            <div className="w-[auto]">
                                ...
                                >
                                    ...
                                    {/* {
                                        loadingDelete ? <Loader color="#fff" size={12} />
                                            : <><IconNewTrash />
                                                <span>
                                                    {t('delete')}
                                                </span></>} */}
        ...
        <div>
            ...
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <Link href="/hrm/leaving_late_early" className="text-primary hover:underline">
            ...
            <div className="panel mt-6">
                <div className="flex md:items-center justify-between md:flex-row flex-col mb-4.5 gap-5">
                    <div className="flex items-center flex-wrap">
                        {/* <RBACWrapper
                            permissionKey={[
                                'leavingLateEarly:create'
                            ]}
                            type={'AND'}
                        > */}
                            <Link href="/hrm/leaving-late-early/create">
                    ...
                    <div className='flex flex-row gap-2'>
                        ...
                        <div className='flex flex-1' style={{ alignItems: "flex-start", position: 'relative' }}>
                            <Flatpickr
