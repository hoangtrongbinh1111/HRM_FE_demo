...
import { LeavingLateEarlyApprove, CreateLeavingLateEarly, GetLeavingLateEarly, EditLeavingLateEarly } from '@/services/apis/leaving-late-early.api';
...
import { listAllTask } from '@/services/apis/task.api';
...
const DetailPage = ({ ...props }: Props) => {
    ...
    }, [data?.createdBy?.department?.id, data?.createdBy?.position?.id, router.query.id, userData?.data?.department?.id, userData?.data?.position.id])

    useEffect(() => {
        dispatch(setPageTitle(disable ? `${t('detail_leaving_late_early')}` : (data ? t('update_leaving_late_early') : t('add_leaving_late_early'))));
    });
    ...
    const handleData = () => {
        if (Number(router.query.id)) {
            GetLeavingLateEarly({ id: router.query.id })
                .then((res) => {
                    setData(res.data);
                    setIsLoading(false);
                })
    ...
    useEffect(() => {
        setInitialValue({
            ...
            startTime: data ? moment(data?.startTime, 'HH:mm:ss').format('HH:mm:ss') : moment(new Date()).format('HH:mm:ss'),
            endTime: data ? moment(data?.endTime, 'HH:mm:ss').format('HH:mm:ss') : moment(new Date()).format('HH:mm:ss'),
            reason: data?.reason ?? '',
            currentApprover: data?.approvalHistory?.find((his: any) => his.status === 'APPROVED')?.approver?.fullName ?? data?.currentApprover?.fullName,
            registrationDay: data ? data?.registrationDay : new Date(),
    ...
    const SubmittedForm = Yup.object().shape({
        ...
        //     console.log(schema)
        //     return startTime && schema.min(startTime, `${t('endtime_must_after_starttime')}`);
        // }),
    ...
    const columnHistorys = [
        ...
        },
        {
            ...
            render: ({ action, sign }: any) => {
                return <span>
                    {
                        action === 'FORWARD' || action === 'SUBMIT' ? (sign === 2 ? 'Ký và Trình lên' : 'Ký nháy và Trình lên') : action === 'REJECT' ? 'Từ chối' : 'Phê duyệt'
                    }
                </span>
            },
            sortable: false,
    ...
    const handleCancel = () => {
        router.push(`/hrm/leaving-late-early?page=${router?.query?.page ?? 1}&perPage=${router?.query?.perPage ?? 10}`);
    };
    ...
    const handleLeavingLateEarly = (param: any) => {
        ...
        if (data) {
            EditLeavingLateEarly({ id: data?.id, ...dataSubmit })
                .then((res) => {
                    showMessage(`${t('update_letter_success')}`, 'success');
                    router.push(`hrm/leaving-late-early?page=${router?.query?.page}&perPage=${router?.query?.perPage}`);
                })
                .catch((err) => {
            ...
            CreateLeavingLateEarly(dataSubmit)
                .then((res) => {
    ...
    const Approve = () => {
        ...
        LeavingLateEarlyApprove({ id: router.query.id, sign: 3 })
            .then(() => {
                handleCancel();
    ...
    const handleApprove = () => {
        ...
        swalDeletes
            .fire({
                // icon: 'question',
                title: `${t('approve')}`,
                text: `${t('approve')} ${t('leaving_late_early')}`,
                padding: '2em',
        ...
        <div>
            ...
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <Link href="/hrm/leaving-late-early" className="text-primary hover:underline">
                        <span>{t('leaving_late_early')}</span>
                    </Link>

                </li>
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <span>
                        {!disable && (data ? t('update_leaving_late_early') : t('add_leaving_late_early'))}
                        {
                            disable && t('detail_leaving_late_early')
                        }
                    </span>
                </li>
            </ul>
                ...
                <div className="flex" style={{ alignItems: 'center' }}>
                    {data?.status !== 'DRAFT' &&
                        ...
                        Number(data?.createdById) !== userData?.data?.id && (
                            <RBACWrapper permissionKey={['leavingLateEarly:update']} type={'AND'}>
                    ...
                    {disable &&
                        (data?.status === LIST_STATUS.DRAFT || (userData?.data?.id === data?.currentApproverId && data?.status !== LIST_STATUS.APPROVED && data?.status !== LIST_STATUS.REJECTED)) && (
                            <RBACWrapper permissionKey={['leavingLateEarly:update']} type={'AND'}>
                                <Link href={`/hrm/leaving-late-early/${router?.query.id}?page=${router?.query?.page}&perPage=${router?.query?.perPage}`}>
                                    <button className="edit-page-btn btn btn-primary h-9 ltr:ml-4 rtl:mr-4">{t('edit')}</button>
                                </Link>
                            </RBACWrapper>
                        )}
                    <Link href={`/hrm/leaving-late-early?page=${router?.query?.page ?? 1}&perPage=${router?.query?.perPage ?? 10}`}>
            ...
            <div className="mb-5">
                <div className="font-semibold">
                                        ...
                                        <Form className="space-y-5 p-4">
                                            ...
                                            <div className="flex justify-between gap-5">
                                                ...
                                                <div className="mb-5 w-1/2">
                                                    ...
                                                    {submitCount ? (
                                                        dayjs(values?.startTime).isAfter(values.startTime) ? (
                                            ...
                                            <div className="flex justify-between gap-5">
                                                ...
                                                <div className="mb-5 w-1/2">
                                                    ...
                                                    <Field autoComplete="off" as="textarea" name="reason" id="reason" disabled={disable} placeholder={`${t('fill_reason')}`} className="form-input" />
                                            ...
                                            {data?.approvalHistory?.find((e: any) => e.status === 'REJECTED')?.comment && (
                                                <div className="flex justify-between gap-5">
                                                    <div className="flex-1">
                                                        ...
                                                        <Field
                                                            ...
                                                            autoComplete="off"
                                                            as="textarea"
                                                            value={data?.approvalHistory?.find((e: any) => e.status === 'REJECTED')?.comment ?? ''}
                    ...
                    <SwitchBtn
                        ...
                        id={id}
                    />
                    {/* <div className="mt-4 flex items-center justify-end ltr:text-right rtl:text-left">
                        {!disable && (
                            <button type="button" className="btn btn-outline-danger cancel-button" onClick={() => handleCancel()}>
                                {t('cancel_form')}
                            </button>
                        )}
                        {(((data?.status === 'DRAFT' || data?.status === 'REJECTED') && data?.createdBy?.id === id) ||
                            router.query.id === 'create' ||
                            (userData?.data?.id === data?.currentApproverId && data?.status !== 'APPROVED')) &&
                            !disable && (
                                <>
                                    <button
